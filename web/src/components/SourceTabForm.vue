<template>
  <el-tabs id="source-edit">
    <el-tab-pane v-for="{ name, children } in Object.values(config)" :label="name" :key="name">
      <el-form label-position="right" label-width="5em">
        <el-form-item
          v-for="{ type, title, namespace, id, array, hint, required } in children"
          :label="title"
          :key="title"
          :required="required"
        >
          ...
          <el-input-number v-if="type == 'Number'" v-model="currentSource[id]" :min="0" />

          <el-select v-if="type == 'Array'" v-model="currentSource[id]">
            <el-option v-for="(name, index) in array" :value="index" :key="name" :label="name" />
          </el-select>
        </el-form-item>
      </el-form>
    </el-tab-pane>
